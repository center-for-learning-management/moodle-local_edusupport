...
$cms = $DB->get_records_sql('SELECT cm.id,cm.instance,cm.course FROM {course_modules} cm, {modules} m WHERE cm.course=? AND cm.module=m.id AND cm.deletioninprogress=0 AND m.name="forum"', [$COURSE->id]);
$forums = [];
